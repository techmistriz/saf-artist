...
class Pincode extends MasterModel
{
    use HasFactory;
    use SoftDeletes;
    ...
    public function getList($data, $with = [], $where = []){  
        ...
        if(!empty($data['query']['search'])){
            ...
            $records->where(function($query) use ($searchKey){
                $query->where('pincode', 'LIKE', '%'.$searchKey.'%')
                ...
                ->orWhereHas('state', function ($query) use ($searchKey) {
                     $query->where('state_name', 'LIKE', '%'.$searchKey.'%');
                })
                ->orWhereHas('city', function ($query) use ($searchKey) {
                     $query->where('city_name', 'LIKE', '%'.$searchKey.'%');
                });
            });
        }
